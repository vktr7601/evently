...
public class BookingServiceExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleProcessOrderException(ProcessOrderException ex, HttpServletRequest request) {
        ErrorResponse error = ErrorResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message(ex.getMessage()).errorCode("PROCESS_ORDER_FAILED").timestamp(LocalDateTime.now()).path(request.getRequestURI()).build();
    ...
    public ResponseEntity<ErrorResponse> handleTicketNotRefundableException(TicketNotRefundableException ex, HttpServletRequest request) {
        ErrorResponse error = ErrorResponse.builder().status(HttpStatus.BAD_REQUEST.value()).message(ex.getMessage()).errorCode("TICKET_NOT_REFUNDABLE").timestamp(LocalDateTime.now()).path(request.getRequestURI()).build();
    ...
    public ResponseEntity<ErrorResponse> handleBookingUnavailable(BookingUnavailableException ex, HttpServletRequest request) {
        ErrorResponse error = ErrorResponse.builder().status(HttpStatus.CONFLICT.value()).message(ex.getMessage()).errorCode("BOOKING_UNAVAILABLE").timestamp(LocalDateTime.now()).path(request.getRequestURI()).build();
