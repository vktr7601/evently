...
const CreateArtist = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const artistRes = await axiosClient.post(`${ROUTES.ARTISTS.ADMIN_CREATE}`, form, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });

            alert("Artist created and all events scheduled!");
            navigate(ROUTES.ARTISTS.DETAILS(artistRes.data.id));
        } catch (err) {
            handleError(err);
        }
    };
        ...
        <div style={styles.page}>
            <div style={styles.container}>
                <header style={styles.header}>
                    <h1 style={styles.title}>Create Artist</h1>
                </header>

                <form style={styles.formCard}>
                    <section style={styles.section}>
                        <h3 style={styles.sectionTitle}>Artist Details</h3>
                        ...
                        <div style={styles.inputGroup}>
                            <label style={styles.label}>Artist Bio</label>
                            <textarea
                                placeholder="Describe the artist — background, achievements, style..."
